...
public function store(Request $request)
    {
        ...
        // 5. Buat record baru di database (Tangkap datanya ke variabel $sertifikat)
        $sertifikat = SertifikatKompetensi::create($validatedData);
        ...
        $urlTujuan = route('sertifikat-kompetensi.index');
        ...
        if ($verifikators->isNotEmpty()) {
            Notification::send(
                ...
                new SubmisiBaruNotification(
                    $sertifikat,             // Data
                    'Sertifikat Kompetensi', // Kategori
...
public function verifikasi(Request $request, SertifikatKompetensi $sertifikatKompetensi)
    {
        ...
        foreach (Auth::user()->unreadNotifications as $notif) {
            // Hapus jika ID cocok dan kategorinya Sertifikat Kompetensi
            if (
                isset($notif->data['item_id']) && 
                $notif->data['item_id'] == $sertifikatKompetensi->id &&
                $notif->data['kategori'] == 'Sertifikat Kompetensi' // Kategori disesuaikan
            ) {
