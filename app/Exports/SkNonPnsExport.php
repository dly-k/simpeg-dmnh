...
class SkNonPnsExport implements FromCollection, WithHeadings, WithMapping, ShouldAutoSize, WithTitle, WithEvents
{
    protected $pegawai;
    protected $search;
    protected $tahun;
    ...
    {
        return SkNonPns::where('pegawai_id', $this->pegawai->id)
            ->when($this->search, function ($query, $search) {
                $query->where('nomor_sk', 'like', "%{$search}%")
                      ->orWhere('jenis_sk', 'like', "%{$search}%");
            })
    ...
    {
        return ['No', 'Jenis SK', 'Nomor SK', 'Tanggal SK', 'Tanggal Mulai', 'Tanggal Selesai', 'Link Berkas'];
    }
    ...
    public function map($row): array
    {
        ...
        return [
            ...
            $row->tanggal_selesai ? Carbon::parse($row->tanggal_selesai)->isoFormat('D MMMM YYYY') : '-',
    ...
    public function registerEvents(): array
    {
        return [
            AfterSheet::class => function (AfterSheet $event) {
                ...
                $sheet->setCellValue('A1', 'RIWAYAT SK NON PNS - ' . strtoupper($this->pegawai->nama_lengkap));
