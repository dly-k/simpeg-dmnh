document.addEventListener("DOMContentLoaded", () => {
  ...
  const initDeleteModal = () => {
    ...
    const bsDeleteModal = modalKonfirmasiHapusEl ? new bootstrap.Modal(modalKonfirmasiHapusEl) : null;

    document.body.addEventListener("click", (event) => {
      const target = event.target;

      // Tombol Lihat Detail
      if (target.closest(".btn-lihat-detail-sk")) {
        event.preventDefault();
        const data = target.closest(".btn-lihat-detail-sk").dataset;
