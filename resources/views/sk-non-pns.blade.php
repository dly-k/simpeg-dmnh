...
.search-filter-row {
  display: flex;
  flex-wrap: nowrap;       /* jangan turun ke bawah kalau masih muat */
  gap: 1rem;
  align-items: center;     /* biar rata tengah vertikal */
  justify-content: flex-start;
 margin-bottom: 1.5rem; /* atau 40px */
}

.search-box {
  flex: 1 1 auto;          /* fleksibel tapi bisa mengecil */
  min-width: 200px;        /* jangan kependekan */
  max-width: 100%;         /* jangan meluber */
}

.filter-select {
  flex: 0 0 180px;         /* fix width untuk filter */
}
...
.btn-lihat { background-color: var(--info); }
...
.upload-area p { margin-top: 1rem; color: #6c757d; }
...
@media (max-width: 991px) {
  ...
  .title-bar,
  .main-content,
  .footer-custom { margin-left: 0 !important; }
  .time-date {
    flex-direction: column;
    gap: 6px;
