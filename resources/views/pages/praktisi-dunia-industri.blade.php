      ...
      <div class="main-content">
        <div class="card">
          <div class="card-body p-4">
            ...
            <!-- Search & Filter -->
            <form action="{{ route('praktisi.index') }}" method="GET" id="filterForm" class="d-flex flex-wrap align-items-center mb-3 gap-2">
              <div class="d-flex flex-grow-1 gap-2">
                ...
                <!-- Filter Semester -->
                <select class="form-select filter-select" name="semester" onchange="this.form.submit()">
              ...
              <div class="ms-auto d-flex gap-2">
                <!-- Export Excel -->
                <a href="{{ route('praktisi.export', request()->all()) }}" class="btn btn-export fw-bold">
                  <i class="fa fa-file-excel me-2"></i> Export Excel
                </a>

                <!-- Tambah Data -->
                <button type="button" class="btn btn-tambah fw-bold" data-bs-toggle="modal" data-bs-target="#pengalamanKerjaModal">
              ...
              <table class="table table-hover table-bordered">
                ...
                <tbody>
                  @forelse ($praktisis as $index => $praktisi)
                    <tr>
                      <td class="text-center">{{ $praktisis->firstItem() + $index }}</td>
                      <td>{{ $praktisi->pegawai->nama_lengkap ?? 'Pegawai Tidak Ditemukan' }}</td>
                      <td>{{ $praktisi->instansi }}</td>
                      <td>{{ $praktisi->jenis_pekerjaan }}</td>
                      <td class="text-center">{{ \Carbon\Carbon::parse($praktisi->tmt)->isoFormat('DD MMM YYYY') }}</td>
                      <td class="text-center">{{ \Carbon\Carbon::parse($praktisi->tst)->isoFormat('DD MMM YYYY') }}</td>
                      ...
                      <td class="text-center">
                        <div class="d-flex gap-2 justify-content-center">
                          ...
                          <a href="#" class="btn-aksi btn-edit" title="Edit Data"
                             data-bs-toggle="modal" data-bs-target="#editPengalamanKerjaModal"
                             data-id="{{ $praktisi->id }}"
                             data-url="{{ route('praktisi.show', $praktisi->id) }}"
                             data-update-url="{{ route('praktisi.update', $praktisi->id) }}">
                            <i class="fa fa-edit"></i>
                          </a>
                          <a href="#" class="btn-aksi btn-hapus btn-hapus-data" title="Hapus Data"
                  ...
                  @empty
                    <tr>
                      <td colspan="9" class="text-center text-muted">
                        Data Praktisi Dunia Industri belum tersedia
                      </td>
                    </tr>
                  @endforelse
                </tbody>
            ...
            <!-- Pagination -->
            {{ $praktisis->appends(request()->query())->links('pagination::bootstrap-5') }}
          </div>
        </div>
      </div>
