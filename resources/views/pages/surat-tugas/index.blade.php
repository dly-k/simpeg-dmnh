    ...
    <div class="main-wrapper">
      ...
      <div class="main-content">
        <div class="table-card">
          ...
          <form action="{{ route('surat-tugas.index') }}" method="GET" id="searchForm"
            ...
            <div class="input-group search-box bg-white flex-grow-1">
              ...
              <input type="text" name="q" value="{{ request('q') }}" id="searchInput"
                    class="form-control border-start-0 search-input" placeholder="Cari Data ...." />
            </div>

            <!-- Filter Semester -->
            <select name="semester" class="form-select filter-select" style="width:200px"
                    onchange="this.form.submit()">
              <option value="">Semua Semester</option>
              @foreach($semesters as $s)
                <option value="{{ $s['tahun'] }}-{{ $s['tipe'] }}" 
                  {{ request('semester') == $s['tahun'].'-'.$s['tipe'] ? 'selected' : '' }}>
          ...
          <div class="d-flex gap-2">
            <a href="{{ route('surat-tugas.export', request()->all()) }}" 
                ...
                @forelse($data as $index => $item)
                  <tr>
                    ...
                    <td>
                      ...
                      {{ $item->tgl_surat_instansi ? $item->tgl_surat_instansi->format('d M Y') : '-' }}
                    </td>
                    <td>
                      {{ $item->no_surat_kadep ?? '-' }}<br>
                      {{ $item->tgl_surat_kadep ? $item->tgl_surat_kadep->format('d M Y') : '-' }}
                    </td>
                    ...
                    <td class="text-center">
                      <div class="d-flex gap-2 justify-content-center">
                        ...
                        <a href="#" class="btn-aksi btn-hapus" data-id="{{ $item->id }}" data-nama="{{ $item->nama_dosen }}">
                            <i class="fas fa-trash"></i>
                        </a>
                      </div>
                    </td>
                  </tr>
                  @include('pages.surat-tugas.edit', ['item' => $item])
                @empty
                  <tr>
