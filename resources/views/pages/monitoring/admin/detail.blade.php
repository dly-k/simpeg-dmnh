    ...
    <div class="main-wrapper">
        ...
        <div class="alert bg-white shadow-sm border-0 d-flex align-items-center mb-4 p-3" style="border-radius: 12px; border-left: 5px solid #001f3f !important;">
            ...
            <div class="flex-grow-1">
                <h5 class="fw-bold mb-1" style="color: #001f3f;">{{ $dosen['nama'] }}</h5>
                <div class="d-flex flex-wrap gap-3">
                    <span class="text-muted small"><i class="fas fa-id-card me-1"></i> NIP. {{ $dosen['nip'] }}</span>
                    <span class="text-muted small"><i class="fas fa-briefcase me-1"></i> Jabatan: <strong>{{ $dosen['jabatan_terakhir'] }}</strong></span>
                    <span class="text-muted small"><i class="fas fa-bullseye me-1"></i> Target: <strong class="text-primary">{{ $dosen['target'] }}</strong></span>
                    <span class="text-danger small fw-bold"><i class="fas fa-clock me-1"></i> Estimasi Pensiun: {{ $dosen['tgl_pensiun'] }}</span>
                </div>
            </div>
        </div>
                ...
                <div class="table-card p-4 h-100">
                    ...
                    <div class="mb-4">
                        ...
                        <div class="d-flex align-items-center mt-1">
                            ...
                            <h6 class="fw-bold mb-0 text-dark">{{ $dosen['target'] }}</h6>
                        </div>
                        <p class="text-muted small mt-1 ps-4">Syarat Minimum: <strong>{{ $targetKUM }} KUM</strong></p>
                    </div>

                    <div class="text-center py-4 mb-4 bg-light rounded-3 border">
                        <h1 class="display-4 fw-bold text-primary mb-0">{{ $currentKUM }}</h1>
                        <p class="text-muted small text-uppercase">Total KUM Saat Ini</p>
                    </div>

                    <div class="p-3 mb-4 rounded-3 border {{ $currentKUM >= $targetKUM ? 'bg-success-subtle text-success border-success' : 'bg-warning-subtle text-warning border-warning' }}">
                        <div class="d-flex align-items-center justify-content-center mb-1">
                            <i class="fas {{ $currentKUM >= $targetKUM ? 'fa-check-circle' : 'fa-exclamation-triangle' }} me-2 fa-lg"></i>
                            <span class="fw-bold text-uppercase">{{ $currentKUM >= $targetKUM ? 'Layak' : 'Belum Layak' }}</span>
                        </div>
                        
                        @if($currentKUM < $targetKUM)
                            <div class="text-center mt-2 pt-2 border-top border-warning border-opacity-25">
                                <span class="small text-dark">Kekurangan:</span>
                                <h4 class="fw-bold text-danger mb-0">{{ $targetKUM - $currentKUM }} <span class="small">KUM</span></h4>
                            </div>
                        @else
                    ...
                    <form action="#" method="POST">
                        ...
                        <div class="mb-3">
                            ...
                            <div class="input-group">
                                <input type="number" step="0.01" class="form-control" name="nilai_skp" placeholder="0.00">
