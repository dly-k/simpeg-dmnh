    ...
    <div class="main-wrapper">
        ...
        <div class="main-content">
            <div class="row g-4">
                ...
                @php 
                    // 1. Definisikan logika status utama di level atas agar bisa diakses kedua kolom
                    $targetAda = ($targetKUM ?? 0) > 0; // Cek apakah dosen punya target saat ini
                    
                    $isKUMOk = (($currentKUM ?? 0) >= ($targetKUM ?? 0) && $targetAda);
                    $isKonversiOk = (($currentKonversi ?? 0) >= ($targetKonversi ?? 0) && $targetAda);
                ...
                {{-- ========================================== --}}
                {{-- KOLOM 1: AUDIT KELAYAKAN (NILAI AK) --}}
                {{-- ========================================== --}}
                <div class="col-lg-5">
                    <div class="table-card p-4 shadow-sm border-0 bg-white h-100" style="border-radius: 15px;">
                        <h5 class="fw-bold mb-4 text-navy"><i class="fas fa-user-shield me-2"></i>Audit Kelayakan Saya</h5>
                        
                        @if(!$targetAda)
                            {{-- TAMPILAN JIKA TARGET KOSONG / KENAIKAN SELESAI --}}
                            <div class="text-center py-4">
                                <div class="mb-3">
                                    <i class="fas fa-medal text-warning" style="font-size: 4rem;"></i>
                                </div>
                                <h5 class="fw-bold text-success">Kenaikan Jabatan Selesai</h5>
                                <p class="text-muted mb-3">
                                    Selamat! Anda saat ini telah menjabat di posisi baru. Nilai Angka Kredit dan berkas lama telah diarsipkan.
                                </p>
                                <div class="alert alert-info border-0 rounded-3 small text-start">
                                    <i class="fas fa-info-circle me-1"></i> Silakan hubungi Admin Kepegawaian jika Anda ingin mulai memantau target kelayakan untuk pangkat berikutnya.
                                </div>
                            </div>
                        @else
                            {{-- TAMPILAN JIKA ADA TARGET --}}
                ...
                {{-- ========================================== --}}
                {{-- KOLOM 2: KELENGKAPAN BERKAS PERSYARATAN --}}
                {{-- ========================================== --}}
                <div class="col-lg-7">
                    <div class="table-card h-100 shadow-sm border-0 bg-white" style="border-radius: 15px;">
                        <div class="tab-bar-container border-bottom px-4 pt-4 pb-3">
                            <h5 class="fw-bold text-navy mb-0"><i class="fas fa-folder-open me-2"></i>Kelengkapan Berkas Persyaratan</h5>
                        </div>
                        ...
                        <div class="p-4">
                            @if(!$targetAda)
                                ...
                                <div class="text-center py-5 mt-2">
                                    <i class="fas fa-file-archive text-secondary mb-3" style="font-size: 3.5rem; opacity: 0.3;"></i>
                                    <h6 class="fw-bold text-muted">Tidak Ada Dokumen yang Diperlukan</h6>
                                    <p class="text-muted small mb-0 px-4">Semua berkas persyaratan pada periode sebelumnya telah diarsipkan. Daftar persyaratan baru akan otomatis muncul setelah target jabatan Anda ditetapkan oleh Admin.</p>
                                </div>
                            @else
                                                    ...
                                                    <div class="d-flex gap-2 justify-content-center">
                                                        @if($req['is_uploaded'])
                                                            ...
                                                            @else
                                                                ...
                                                                <span class="btn btn-sm btn-light disabled text-muted shadow-sm" style="opacity: 0.8;">
                                                ...
                                                <div class="modal-dialog modal-dialog-centered">
                                                    <form action="{{ route('efile.store', $pegawai->id) }}" method="POST" enctype="multipart/form-data">
                                                        ...
                                                        <div class="modal-content">
                                                            <div class="modal-header {{ $req['is_uploaded'] ? 'bg-warning text-dark' : 'bg-primary text-white' }}">
                                                                <h5 class="modal-title fw-bold">
                                                                    <i class="fas fa-file-upload me-2"></i>
                                                                    {{ $req['is_uploaded'] ? 'Ganti/Revisi Berkas' : 'Unggah Berkas' }}
                                                                </h5>
                                                                <button type="button" class="btn-close {{ $req['is_uploaded'] ? '' : 'btn-close-white' }}" data-bs-dismiss="modal"></button>
                                                            </div>
